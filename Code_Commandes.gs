...
const REGISTRE_SHEET_ID = 'A_COMPLETER_APRES_initRegistre';
...
function listProduits(siteKey, fournisseur) {
  const ss = openSiteSheet(siteKey);
  const sheet = ss.getSheetByName(fournisseur);
  if (!sheet) throw new Error('Onglet fournisseur introuvable : ' + fournisseur);

  const values = sheet.getDataRange().getValues();
  const IGNORED = ['designation produit', 'total ht', 'dont consommable', 'dont investissement',
                   'dont maintenance', 'dont projet-pta', 'dont abonnement', 'dont épreuves'];

  const items = [];
  values.forEach(row => {
    const designation = String(row[0] || '').trim();
    if (!designation) return;
    if (IGNORED.indexOf(designation.toLowerCase()) !== -1) return;
    if (designation.startsWith('(')) return;
